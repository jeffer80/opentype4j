...
public class Font extends AbstractParser<Font> {
	...
	public Path getPath(String text, Float x, Float y, Float fontSize, Float letterSpacing) {
		ScriptObjectMirror obj = (ScriptObjectMirror) (scriptObjectMirror.callMember("getTextPath", text, x, y, fontSize, letterSpacing));
		return new Path().parse(obj);
	}
	...
	public Path getPath(List<GlyphData> glyphs, FontCell fontCell, Integer maxFontPerLine) {
		...
		for (int i = 0; i < length; i++) {
			...
			Path curPath = glyphData.getPath(new FontCell(fontCell.getWidth(), fontCell.getHeight(),
					fontCell.getWidth() * (i % maxFontPerLine), fontCell.getHeight() * (i / maxFontPerLine)));
			if (path == null) {
				path = curPath;
			} else {
	...
	/**
	 * Convert the given character to a single glyph index. Note that this function
	 * assumes that there is a one-to-one mapping between the given character and a
	 * glyph; for complex scripts this might not be the case.
	 *
	 * @param charCode
	 * @return
	 */
	...
	/**
	 * Convert the given character to a single Glyph object. Note that this function
	 * assumes that there is a one-to-one mapping between the given character and a
	 * glyph; for complex scripts this might not be the case.
	 *
	 * @param charCode
	...
	/**
	 * Gets the glyph name by index {@link GlyphNames#glyphIndexToName(int)}
	 *
	 * @param gid
	 * @return
	 */
	...
	public String toString() {
		return "Font{" + "numGlyphs=" + numGlyphs + ", isCIDFont=" + isCIDFont + ", defaultWidthX=" + defaultWidthX
				+ ", nominalWidthX=" + nominalWidthX + ", unitsPerEm=" + unitsPerEm + ", ascender=" + ascender
				+ ", descender=" + descender + ", outlinesFormat='" + outlinesFormat + '\'' + ", glyphNames="
				+ glyphNames + ", glyphs=" + glyphs + ", tables=" + tables + '}';
	}
}
